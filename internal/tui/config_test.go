...
func TestNewConfigModel(t *testing.T) {
	m := NewConfigModel(installer.AppCustomerPortal)
...
func TestNewConfigModelPoller(t *testing.T) {
	m := NewConfigModel(installer.AppPoller)
	if len(m.fields) != 2 {
...
func TestNewConfigModelFreeRADIUS(t *testing.T) {
	m := NewConfigModel(installer.AppFreeRADIUS)
	if len(m.fields) != 1 {
...
func TestConfigModelTabWraps(t *testing.T) {
	m := NewConfigModel(installer.AppPoller) // 2 fields
	m, _ = m.Update(tea.KeyMsg{Type: tea.KeyTab})
	m, _ = m.Update(tea.KeyMsg{Type: tea.KeyTab})
	if m.FocusIndex() != 0 {
...
func TestConfigModelSubmitValidation(t *testing.T) {
	m := NewConfigModel(installer.AppPoller) // sonar_url + poller_api_key
